...
export default function NotableProjectCard({ project, index }: { project: Project; index: number }) {
      ...
      <div className={`space-y-8 ${isEven ? "lg:order-1" : ""}`}>
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {project.achievements.slice(0, labels.bulletLimit).map((achievement, idx) => (
        ...
        <div className="space-y-4">
          ...
          <div className="border-t border-b border-[#484848] divide-y divide-[#484848]">
            ...
            {project.techStack.length > 0 && <InfoRow label={labels.techStackLabel} value={project.techStack.slice(0, labels.techLimit).join(", ")} />}
          </div>
        </div>
          ...
          <div className="flex gap-6 flex-wrap">
            <a href={project.url} target="_blank" rel="noopener noreferrer" className="group inline-flex flex-col gap-1">
              <div className="flex items-center gap-1">
                <span className="font-['Inter',sans-serif] font-bold text-[#d3e97a] text-[14px] uppercase">{projectLinkLabel(project)}</span>
